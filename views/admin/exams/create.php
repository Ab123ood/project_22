...
<div class="flex items-center justify-between mb-6">
    <h2 class="text-xl md:text-2xl font-bold text-gray-900">إنشاء اختبار جديد</h2>
...
<form method="post" action="<?= $basePath ?>/admin/exams" id="examForm" class="max-w-5xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="title" value="<?= htmlspecialchars($old['title'] ?? '') ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="مثال: اختبار التصيد الإلكتروني" required>
            </div>
            ...
            <div>
                ...
                <input type="number" name="duration_minutes" value="<?= htmlspecialchars($old['duration_minutes'] ?? '30') ?>" min="5" max="120" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="30" required>
            </div>
        </div>

        <div class="mt-6">
            <label class="block text-sm font-medium text-gray-700 mb-2">وصف الاختبار</label>
            <textarea name="description" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="وصف مختصر عن محتوى الاختبار وأهدافه..."><?= htmlspecialchars($old['description'] ?? '') ?></textarea>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="number" name="passing_score" value="<?= htmlspecialchars($old['passing_score'] ?? '70') ?>" min="50" max="100" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="70">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">عدد المحاولات المسموحة</label>
                <input type="number" name="max_attempts" value="<?= htmlspecialchars($old['max_attempts'] ?? '3') ?>" min="1" max="10" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="3">
            </div>
        </div>
    ...
    <div class="bg-blue-50 border border-blue-200 text-blue-800 rounded-lg p-4 mb-6 text-sm">
        بعد إنشاء الاختبار سيتم تحويلك تلقائياً إلى صفحة "أسئلة الاختبار" لإضافة الأسئلة والخيارات.
    </div>
    ...
    <div class="flex items-center justify-between bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div class="flex space-x-3 space-x-reverse">
            <button type="submit" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg">إنشاء الاختبار</button>
        </div>
    </div>
</form>
