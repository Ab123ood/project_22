...
<div class="flex items-center justify-between mb-6">
    <h2 class="text-xl md:text-2xl font-bold text-gray-900">Assessment modification</h2>
...
<form method="post" action="<?= $basePath ?>/admin/exams/update" id="examForm" class="max-w-5xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="title" value="<?= htmlspecialchars($exam['title'] ?? '') ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Example: E -hunting assessment " required>
            </div>
            ...
            <div>
                ...
                <input type="Count" name="duration_minutes" value="<?= htmlspecialchars($exam['duration_minutes'] ?? '30') ?>" min="5" max="120" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="30" required>
            </div>
        </div>

        <div class="mt-6">
            <label class="block text-sm font-medium text-gray-700 mb-2">Assessment description</label>
            <textarea name="description" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="A brief description of the assessment content and its goals ... "><?= htmlspecialchars($exam['description'] ?? '') ?></textarea>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="Count" name="passing_score" value="<?= htmlspecialchars($exam['passing_score'] ?? '70') ?>" min="50" max="100" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="70">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">The Count of permissible attempts</label>
                <input type="Count" name="max_attempts" value="<?= htmlspecialchars($exam['max_attempts'] ?? '3') ?>" min="1" max="10" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="3">
            </div>
        </div>
    ...
    <div class="flex items-center justify-between bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <div class="flex space-x-3 space-x-reverse">
            <a href="<?= $basePath ?>/admin/exams/questions?exam_id=<?= $exam['id'] ?? '' ?>" class="px-6 py-3 bg-green-600 hover:bg-green-700 text-white rounded-lg">Questions management</a>
